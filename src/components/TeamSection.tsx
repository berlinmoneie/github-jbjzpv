...
export default function TeamSection() {
  ...
  // For mobile view, we'll show Rosangela in both locations
  const getMobileTeam = (location: string) => {
    if (location === 'solna') {
      return solnaTeam;
    } else if (location === 'sodermalm') {
      return [...sodermalmTeam, solnaTeam[0]]; // Add Rosangela to Södermalm team
    }
    return [];
  };

  // For desktop view, we'll show all team members with Rosangela appearing once
  const desktopTeam = [...solnaTeam, ...sodermalmTeam]; // Include all team members
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              className={`p-6 rounded-xl text-left transition-all duration-300 relative overflow-hidden shadow-lg
                ...
                ${selectedLocation === 'solna' && 'ring-2 ring-rose-500/20'}`}
            >
            ...
            <button
              ...
              className={`p-6 rounded-xl text-left transition-all duration-300 relative overflow-hidden shadow-lg
                bg-gradient-to-r from-rose-500 to-lavender-500 text-white
                ${selectedLocation === 'sodermalm' && 'ring-2 ring-rose-500/20'}`}
            >
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-2">
                  <MapPin className="w-5 h-5" />
